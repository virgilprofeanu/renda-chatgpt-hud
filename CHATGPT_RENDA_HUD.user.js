...
(() => {
  ...
  function localCanonSelect(text) {
    ...
    return {ok: true, source: 'local', reflexe: localPickReflexes(text), norme: localPickNorme(text, ds), directive: []};
  }
  ...
  function gmCanonSelect(text, cb) {
    ...
    gm({
      ...
      timeout: 900,
      onload: (res) => { try { const d = JSON.parse(res.responseText); cb(d && d.ok ? d : localCanonSelect(text)); } catch (_) { cb(localCanonSelect(text)); } },
