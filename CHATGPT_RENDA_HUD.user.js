...
// @version      4.9.16
// v4.9.16 (2026-07-16): AGENTI WORKSPACE actualizati din lista noua (49 intrari, dedup pe slug
// — pm-proiecte si corector-boq pastreaza ID-ul cel mai nou), categorii stabile pentru intrarile
// existente + clase noi C03/C04/C05, si TAGURI SINONIME per agent (camp t) incluse in cautarea
// panoului (nume/descriere/cluster/taguri).
...
(() => {
  ...
  const BUILD_STAMP = '2026-07-16-14:11:04';   // aaaa-ll-zz-hh:mm:ss — se re-baga la fiecare release
    ...
    function render() {
      ...
      if (tab === 'agents' || tab === 'external') {
        ...
        ((tab === 'agents' ? RV_AGENTS.agents : RV_AGENTS.external) || []).forEach((a) => {
          if (q && ((a.n + ' ' + (a.e || '') + ' ' + (a.c || '') + ' ' + (a.t || '')).toLowerCase().indexOf(q) === -1)) return;
