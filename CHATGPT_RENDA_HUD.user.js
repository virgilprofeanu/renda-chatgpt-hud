...
(() => {
  ...
  const css = String.raw`
    ...
    #${MIS_PANEL_ID} .mis-change { margin-top: 8px; padding: 2px 8px; border: 1px solid #253046; border-radius: 7px; color: #8a93a0; background: transparent; cursor: pointer; font: 10px 'Segoe UI', sans-serif; }
    #${MIS_PANEL_ID} .mis-change:hover { color: #b9dcff; border-color: rgba(55,138,221,.5); }
  ...
  // v4.9.36: vizualizare TEMPORARA "alt angajat" — nu se persista; la refresh revii la tine
  let bootTemp = null;
  ...
  function buildMissionCard(hud) {
    ...
    // flag-uri de UI: selectoarele se deschid explicit din "schimbă categoria…" /
    // "alt angajat…" (altfel auto-match-ul le-ar re-inchide instant)
    let misSelOpen = false;
    let bootSelOpen = false;
    ...
    function bootHtml(my) {
      ...
      if (my && !bootSelOpen) {
        ...
        return '<div class="mis-boot"><div class="mis-boot-t">🚀 BOOT SESSION <span class="mis-src">· ' + esc(my.i) + (my.st ? ' · ' + esc(my.st) : '') + (bootTemp ? ' · vizualizezi ALT angajat' : '') + '</span></div>'
          + '<div class="mis-boot-p">' + esc(my.b.slice(0, 140)) + '…</div>'
          + '<div class="pp-row"><button class="pp-act boot-ins" type="button" title="Inserează textul integral de boot în composer — apoi adaugi sarcina și trimiți tu">⤵ În composer</button>'
          + '<button class="pp-act boot-copy" type="button" title="Copiază textul integral de boot">📋 Copiază</button>'
          + (bootTemp ? '<button class="mis-change boot-me" type="button">înapoi la mine</button>' : '')
          + '<button class="mis-change boot-alt" type="button">alt angajat…</button></div>'
    ...
    function missionBlock(rec, srcLabel) {
      return '<div class="mis-m">' + esc(rec.m) + '</div>'
        + '<ol class="mis-a">' + rec.a.map((a) => '<li>' + esc(a) + '</li>').join('') + '</ol>'
        + '<button class="mis-change" type="button" title="' + esc(srcLabel) + '">schimbă categoria…</button>';
    }
    function render() {
      ...
      if (my) {
        ...
        body.innerHTML = who
          ...
          + '</div>'
          + (rec && !misSelOpen
              ? missionBlock(rec, manual ? 'categorie aleasă manual' : 'derivată din funcția operațională: ' + my.f)
              : '<div class="pp-hint">Alege categoria de misiune:</div>' + misSelHtml(rec ? rec.f : ''))
    ...
    body.addEventListener('click', (ev) => {
      ...
      if (bi) {
        ...
        const ok = insertIntoComposer(bt.b + '\n\n');
        const t = bi.textContent;
        bi.textContent = ok ? '✓ inserat' : '✗ composer negăsit';
        setTimeout(() => { bi.textContent = t; }, 1200);
        rvlog('boot session ' + (ok ? 'inserat in composer' : 'NEINSERAT — composer negasit') + ' (' + bt.b.length + ' caractere)');
        return;
      }
      ...
      if (bc) {
        const bt = findMyBoot();
        if (!bt) return;
        navigator.clipboard.writeText(bt.b).then(() => {
          const t = bc.textContent; bc.textContent = '✓ copiat';
          setTimeout(() => { bc.textContent = t; }, 1200);
        }).catch(() => {});
        return;
      }
      if (ev.target.closest('button.boot-me')) { bootTemp = null; render(); return; }
      if (ev.target.closest('button.boot-alt')) { bootSelOpen = true; render(); return; }
      if (ev.target.closest('button.mis-change')) { misSelOpen = true; render(); }
    });
    body.addEventListener('change', (ev) => {
      const s = ev.target.closest('select.boot-sel');
      if (s && s.value) {
        // v4.9.36: cu identitate automata, alegerea e doar VIZUALIZARE temporara;
        // fara identitate automata, alegerea se persista (e chiar userul)
        if (autoBootMatch()) bootTemp = RV_BOOT.find((x) => x.i === s.value) || null;
        else setBootSel(s.value);
        bootSelOpen = false; render(); return;
      }
      const f = ev.target.closest('select.mis-f-sel');
      if (f && f.value) {
        const cur = findMyBoot();
        setMisF(cur ? cur.i : '_anon', f.value);
        misSelOpen = false; render();
      }
    });
