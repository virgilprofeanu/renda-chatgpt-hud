...
(() => {
  ...
  function buildAgentsPanel(hud) {
    ...
    panel.innerHTML = '<button class="ag-close" type="button" aria-label="Închide">✕</button>'
      + '<h3>🤖 AGENȚI &amp; SKILLS RENDA</h3>'
      + '<div class="ag-tabs">'
      + '<button class="ag-tab on" data-tab="agents" type="button">Agenți GPT (' + nA + ')</button>'
      + '<button class="ag-tab" data-tab="skills" type="button">Skills (' + nS + ')</button>'
      + '</div>'
    ...
    function render() {
      ...
      if (tab === 'agents') {
        ...
        (RV_AGENTS.skills || []).forEach((s) => {
          ...
          html += '<div class="ag-card"><div class="ag-info"><div class="ag-name">' + esc(s.n) + '</div><div class="ag-desc">' + esc(s.d || '') + '</div></div><div class="ag-act"><button type="button" data-copy="' + esc(s.n) + '" title="Copiază numele skill-ului — pomenește-l în mesaj ca să-l activezi">📋 nume</button></div></div>';
        });
      }
